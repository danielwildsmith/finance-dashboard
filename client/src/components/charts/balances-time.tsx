...
export const NetWorthTimeGraph = ({
  data,
}: {
  data: DatedNetWorth[] | null;
}) => {
  if (data) {
    return (
      <>
        ...
        {data.length < 5 ? (
          <Typography
            variant="body2"
            sx={{textAlign: "center", color: "#878fa0"}}
          >
            *The graph displays the most recent 13 weekly balance records
            available and provides more meaningful insights a few weeks after
            account creation.*
          </Typography>
        ) : null}
      </>
